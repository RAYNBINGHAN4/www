    ...
    public function index(){
        if(IS_POST){
            ...
            foreach($fields as &$field){  //使用的引用传值,在foreach中改变field的内容从而影响到fields
                ...
                preg_match("/(.*)@([a-z]*)\|?(.*)/",$comment,$result);//parse_str() 函数把查询字符串解析到变量中。
                if(!empty($result)){
            ...
            //>>php中语法,$field出了循环依然存在!!!!!!!
            unset($field); //避免后面在使用$field出错,因为field也是在foreach中使用(此时$fields包含input_type,option_values等添加的索引)
